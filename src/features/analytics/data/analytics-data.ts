...
const labels: Record<DashboardRange, string[]> = {
  today: ["٦ ص", "٨ ص", "١٠ ص", "١٢ م", "٢ م", "٤ م", "٦ م", "٨ م"],
  week: ["السبت", "الأحد", "الاثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة"],
  month: ["أسبوع ١", "أسبوع ٢", "أسبوع ٣", "أسبوع ٤"],
};
...
export function getAnalyticsData(range: DashboardRange): AnalyticsData {
  ...
  const distributionValues = allocate(total, [0.78, 0.14, 0.05, 0.03]);
  ...
  return {
    ...
    distribution: [
      ...
      { name: "قيد التنفيذ", value: distributionValues[1], color: "#376FF1" },
      { name: "متأخرة", value: distributionValues[2], color: "#F59E0B" },
      { name: "ملغاة", value: distributionValues[3], color: "#16A34A" },
    ],

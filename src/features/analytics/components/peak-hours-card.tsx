...
const hours = ["6ص", "8ص", "10ص", "12م", "2م", "4م", "6م", "8م", "10م", "12ص"];
      ...
      <div className="p-4">
        <div className="space-y-1.5">
          ...
          {values.map((row, rowIndex) => (
            <div key={days[rowIndex]} className="grid grid-cols-[50px_repeat(10,1fr)] gap-1">
              <span className="text-[9px] text-muted-foreground">{days[rowIndex]}</span>
              {row.map((value, columnIndex) => (
                <span
                  key={hours[columnIndex]}
                  title={`${days[rowIndex]} ${hours[columnIndex]}: مستوى نشاط ${value} من 5`}
